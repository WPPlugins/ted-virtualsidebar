...
Plugin URI: http://wp.tedac.fr/en/plugins/ted-virtualsidebar/
	...
	if( is_admin() ){ ?>
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				<tr valign="top">
				<th scope="row"><?php _e('Number of sidebars :', 'ted-virtualsidebar') ?></th>
				...
				for($i=1;$i<= $nbr;$i++){ ?>
				<tr valign="top">
				<th scope="row"><?php _e('Name of the sidebar No :', 'ted-virtualsidebar');echo $i; ?></th>
				<td><input type="text" name="ted_vsb_sidebar_name<?php echo $i; ?>" value="<?php echo get_option('ted_vsb_sidebar_name'.$i); ?>" /></td>
				</tr>
				<tr valign="top">
				<th scope="row"><?php _e('Location of the sidebar No :', 'ted-virtualsidebar');echo $i; ?></th>
				<td>		
				<select name="ted_vsb_location_<?php echo $i; ?>">
					<option value="0" <?php if(get_option('ted_vsb_location_'.$i)=="0") echo " selected"; ?> ><?php _e('Before the LOOP', 'ted-virtualsidebar'); ?></option>
					<option value="1" <?php if(get_option('ted_vsb_location_'.$i)=="1") echo " selected"; ?> ><?php _e('After the LOOP', 'ted-virtualsidebar'); ?></option>
				</select> 
				</td>
				</tr>
				<tr valign="top">
				<th scope="row"><?php _e('On wich templates pages ?', 'ted-virtualsidebar'); ?> :</th>
				<td>		
			...
			<p class="submit">
			<input type="submit" class="button-primary" value="<?php _e('Save All', 'ted-virtualsidebar') ?>" />
			</p>
		</form>
		</div>
<?php 
